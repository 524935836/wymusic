...
Page({
    ...
    onLoad: function (options) {
      ...
      PubSub.subscribe('switchType',(msg,type) => {
        // console.log(type)
        let {recommendList,i} = this.data
        if(type === 'pre'){
          i === 0 && (i = recommendList.length)
          i--
        }else{
          i === recommendList.length - 1 && (i = i = -1)
          i++
        }

        // 将musicId发布给songDetail页面
        let musicId = recommendList[i].id
    ...
    toSongDetail(event){
      ...
      this.setData({
        i: index  //更新data中的index为音乐的下标
      })
      // console.log(index)
      // 跳转并发送歌曲id
      wx.navigateTo({
        url:'/songePackages/pages/songdetail/songdetail?musicId=' + song.id
      })
    },
